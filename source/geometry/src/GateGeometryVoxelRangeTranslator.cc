...
void GateGeometryVoxelRangeTranslator::Describe(G4int) 
{
  ...
  for (G4int iRange = 0; iRange< (G4int)m_voxelMaterialTranslation.size(); iRange++) {
    ...
    G4cout << "\tRange "  << std::setw(3) << iRange 
	   ...
	   << xmax 
	   << " ]  ---> material " << material 
	   << ", visibility " << GetMaterialAttributes(   GateDetectorConstruction::GetGateDetectorConstruction()->GetMaterialDatabase().GetMaterial(material)  )->IsVisible()
	   << ", coulour "    << GetMaterialAttributes(   GateDetectorConstruction::GetGateDetectorConstruction()->GetMaterialDatabase().GetMaterial(material)  )->GetColour()
	   << Gateendl;
  }
}
