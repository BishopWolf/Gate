...
void GateGeometryVoxelTabulatedTranslator::ReadTranslationTable(G4String fileName)
{
  ...
  for (G4int iCol=0; iCol<nTotCol; iCol++) {
    ...
    m_voxelAttributesTranslation[ GateDetectorConstruction::GetGateDetectorConstruction()->GetMaterialDatabase().GetMaterial(material) ]= new G4VisAttributes(visible,G4Colour(red,green,blue,alpha));
