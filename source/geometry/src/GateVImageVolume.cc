  ...
  GateVVolume(name,acceptsChildren,depth)
{
  ...
  pOwnMaterial = GateDetectorConstruction::GetGateDetectorConstruction()->GetMaterialDatabase().GetMaterial("Air");
...
//     mHounsfieldMaterialTable.MapLabelToMaterial(lab2mat);
	...
	// G4Material * mat =
	// 	  GateDetectorConstruction::GetGateDetectorConstruction()->GetMaterialDatabase().GetMaterial(lab2mat[*pi]);
...
void GateVImageVolume::LoadImageMaterialsFromRangeTable()
{
  ...
  m_voxelAttributesTranslation[GateDetectorConstruction::GetGateDetectorConstruction()->GetMaterialDatabase().GetMaterial(material) ] =
...
void GateVImageVolume::BuildLabelToG4MaterialVector( std::vector<G4Material*>& M )
{
  ...
  for (lit=mLabelToMaterialName.begin(); lit!=mLabelToMaterialName.end(); ++lit) {
    ...
    M.push_back(GateDetectorConstruction::GetGateDetectorConstruction()->GetMaterialDatabase().GetMaterial((*lit).second));
    l++;
  }
